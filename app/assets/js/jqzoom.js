;(function($){
	$.fn.zoom = function(options){
		...
		var _option = {
			...
			zoom_area_width: 100, 		
			zoom_area_height: "dustify",
			zoom_area_distance: 4,      
			zoom_easing: true,          
			click_to_zoom: false,
			zoom_element: "auto",
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".bzoom_big_image").length){
			...
			if(!$(".bzoom_small_thumbs").length){
				...
				for(var i=0; i<_option.small_thumbs; i++){
					...
					if(i==0){
						html += '<li class="bzoom_smallthumb_active"><img  class="c_width" src="'+smurl+'" alt="small" /></li>';
					}else{
						html += '<li style="opacity:0.4;"><img src="'+smurl+'" class="c_width" alt="small"/></li>';
					}
				}

				$small = $('<li class="bzoom_small_thumbs" style="top:'+top+'px; width:300px;"><ul class="clearfix" id="thumail_scroll" style="width: 300px;">'+html+'</ul></li>');
				$ul.append($small);
